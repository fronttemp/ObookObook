...
export default async function (
  ...
  try {
    const { data } = await axios.get(
      ...
      `https://www.aladin.co.kr/ttb/api/${search}.aspx?ttbkey=ttbckisss66601645001&SearchTarget=eBook&output=js&Version=20131101&MaxResults=50${query}${queryType}${itemID}${opt}${tag}${sort}${id}
      `
    )
